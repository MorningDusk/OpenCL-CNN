...
__kernel void softmax(__global float* outputs, const int neuron_size)
{
	int i;
	float sum = 0;
	int begin_idx = get_global_id(0) * neuron_size;
...
__kernel void find_max_and_conf(__global float* inputs, __global float* labels, __global float* confidences, const int classNum, const int batchOffset)
{
	...
	int max_idx = 0;
	float max = 0;
	__global float* input = inputs + get_global_id(0) * classNum;
	...
	int loc = get_global_id(0) + batchOffset;
	max_idx += classNum * get_global_id(0);
	labels[loc] = max_idx;
